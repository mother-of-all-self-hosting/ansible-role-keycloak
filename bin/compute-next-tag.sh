...
#
# Tags look like `v<Keycloak version>-<release>`:
#
# - if defaults/main.yml points at a Keycloak version that has never been
#   released, the release counter restarts at 0 (`v26.7.2-0`)
# - otherwise the counter is incremented (`v26.7.2-1`), but only if something
...
version="$(sed -nE 's|^keycloak_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Keycloak version from $defaults_path"
	exit 1
fi

# The Keycloak version values carry no leading `v` (e.g. `26.7.2`) while the
# tags do, and the substitution keeps this correct either way.
